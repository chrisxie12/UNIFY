...
function StatusBar() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 24px 4px', fontSize: 13, fontWeight: 600, color: '#111827' }}>
      <span>9:41</span>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        {/* Signal bars */}
        <svg width="17" height="12" viewBox="0 0 17 12" fill="none">
          <rect x="0"  y="7"  width="3" height="5"  rx="1" fill="#111827"/>
          <rect x="4.5" y="4.5" width="3" height="7.5" rx="1" fill="#111827"/>
        ...
        <svg width="16" height="12" viewBox="0 0 16 12" fill="none">
          ...
          <path d="M3.5 6.5C4.8 5.2 6.3 4.5 8 4.5s3.2.7 4.5 2" stroke="#111827" strokeWidth="1.5" strokeLinecap="round" fill="none"/>
          <path d="M1 3.5C3 1.3 5.4 0 8 0s5 1.3 7 3.5" stroke="#111827" strokeWidth="1.5" strokeLinecap="round" fill="none" opacity="0.4"/>
        </svg>
        {/* Battery */}
        <svg width="25" height="12" viewBox="0 0 25 12" fill="none">
          <rect x="0.5" y="0.5" width="21" height="11" rx="3.5" stroke="#111827" strokeWidth="1"/>
          <rect x="22" y="3.5" width="2.5" height="5" rx="1.5" fill="#111827" opacity="0.4"/>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: '#ffffff',
          borderRadius: 40,
          boxShadow: '0 25px 50px -12px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.06)',
          overflow: 'hidden',
          position: 'relative',
          minHeight: 780,
          ...
          {/* Notch */}
          <div style={{ width: 120, height: 30, background: '#111827', borderRadius: '0 0 20px 20px', margin: '0 auto 0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#1f2937', border: '2px solid #333' }} />
          </div>

          {/* Content */}
          <div style={{ flex: 1, padding: '32px 28px 40px', display: 'flex', flexDirection: 'column', position: 'relative' }}>

            {/* Logo area */}
            <div style={{ textAlign: 'center', marginBottom: 48, animation: mounted ? 'fadeUp 600ms cubic-bezier(0.16,1,0.3,1) both' : 'none', position: 'relative' }}>
              {/* Spark — top right of logo */}
              <div style={{ position: 'absolute', top: -8, right: 32 }}>
                <BlueSpark />
              </div>
              <h1 style={{ fontSize: '2.5rem', fontWeight: 800, color: '#111827', letterSpacing: '-0.02em', margin: 0, lineHeight: 1 }}>
                UNIFY
              </h1>
            ...
            {done ? (
              ...
              <>
                {/* Toggle */}
                <div style={{
                  display: 'flex', width: 280, height: 52, margin: '0 auto 32px',
                  background: '#fff', border: '1.5px solid #E5E7EB', borderRadius: 9999, padding: 4,
                  animation: mounted ? 'fadeUp 600ms cubic-bezier(0.16,1,0.3,1) 200ms both' : 'none',
                }}>
                  {['signup', 'login'].map((m) => (
                    <button
                      key={m}
                ...
                {/* Orange radiate + email input */}
                <div style={{ position: 'relative', animation: mounted ? 'fadeUp 600ms cubic-bezier(0.16,1,0.3,1) 300ms both' : 'none' }}>
                  {/* Radiate marks */}
                  <div style={{ position: 'absolute', left: -8, top: '50%', transform: 'translateY(-50%)', zIndex: 0 }}>
                    <OrangeRadiate />
                  </div>
                ...
                {/* Password input */}
                <div style={{ marginTop: 16, position: 'relative', animation: mounted ? 'fadeUp 600ms cubic-bezier(0.16,1,0.3,1) 400ms both' : 'none' }}>
                ...
                {/* Primary CTA */}
                <div style={{ marginTop: 24, animation: mounted ? 'fadeUp 600ms cubic-bezier(0.16,1,0.3,1) 500ms both' : 'none' }}>
                ...
                {/* Divider */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '32px 0', animation: mounted ? 'fadeUp 600ms cubic-bezier(0.16,1,0.3,1) 560ms both' : 'none' }}>
                ...
                {/* Social buttons */}
                <div style={{ display: 'flex', justifyContent: 'center', gap: 24, animation: mounted ? 'scaleIn 500ms cubic-bezier(0.16,1,0.3,1) 620ms both' : 'none' }}>
                  {/* Apple */}
                  <button className="social-btn" aria-label="Sign in with Apple" onClick={() => console.log('Apple login clicked')}>
                  ...
                  {/* Facebook */}
                  <button className="social-btn" aria-label="Sign in with Facebook" onClick={() => console.log('Facebook login clicked')}>
                  ...
                  {/* Google */}
                  <button className="social-btn" aria-label="Sign in with Google" onClick={() => console.log('Google login clicked')}>
            ...
            {/* Footer links */}
            <div style={{ marginTop: 'auto', paddingTop: 40, textAlign: 'center', fontSize: '0.72rem', color: '#9CA3AF' }}>
