...
-- step12: conversations, channels, messages, reactions, threads, polls, requests, mentions, read receipts
...
CREATE INDEX idx_channels_conv ON channels(conversation_id, position);
...
CREATE INDEX idx_read_receipts_user_conv ON message_read_receipts(user_id, conversation_id);
...
-- Message requests: recipient can see their requests
...
CREATE POLICY blocks_delete ON blocked_users FOR DELETE
  USING (blocker_id = auth.uid());
