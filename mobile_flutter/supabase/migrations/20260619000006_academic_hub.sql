-- UNIFY Academic Hub (NEW schema — Flutter-compatible)
-- All FKs reference profiles(id) not users(id)
-- Replaces old schema: gpa_entries, exam_schedule, study_tasks, assignment_reminders, academic_searches
...
-- ── Indexes ──────────────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_academic_resources_course ON academic_resources(course_id);
CREATE INDEX IF NOT EXISTS idx_academic_resources_type ON academic_resources(type);
CREATE INDEX IF NOT EXISTS idx_academic_resources_verification ON academic_resources(verification_status);
CREATE INDEX IF NOT EXISTS idx_academic_resources_downloads ON academic_resources(download_count DESC);
...
CREATE INDEX IF NOT EXISTS idx_study_plan_items_plan ON study_plan_items(plan_id, scheduled_date);
CREATE INDEX IF NOT EXISTS idx_resource_ratings_resource ON resource_ratings(resource_id);
CREATE INDEX IF NOT EXISTS idx_courses_code ON courses(code);
CREATE INDEX IF NOT EXISTS idx_courses_department ON courses(department);
CREATE INDEX IF NOT EXISTS idx_exam_timetables_course ON exam_timetables(course_id);
CREATE INDEX IF NOT EXISTS idx_resources_search ON academic_resources USING gin(to_tsvector('english', title || ' ' || COALESCE(description, '')));
CREATE INDEX IF NOT EXISTS idx_courses_search ON courses USING gin(to_tsvector('english', name || ' ' || code));
...
DROP POLICY IF EXISTS resources_select ON academic_resources;
CREATE POLICY resources_select ON academic_resources FOR SELECT USING (TRUE);
DROP POLICY IF EXISTS resources_insert ON academic_resources;
CREATE POLICY resources_insert ON academic_resources FOR INSERT WITH CHECK (uploaded_by = auth.uid());
DROP POLICY IF EXISTS resources_update ON academic_resources;
CREATE POLICY resources_update ON academic_resources FOR UPDATE USING (uploaded_by = auth.uid() OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'superadmin')));
